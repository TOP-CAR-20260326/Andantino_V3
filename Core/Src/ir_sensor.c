...
void IR_Init(void)
{
    ...
    /* ⭐ 新增：先做一次 SIDE 讀取，把 marker 狀態同步到當下實際情況，
     *    避免開機若剛好停在 marker 上，第一次主迴圈就誤判為 rising edge */
    IR_ReadSide();
 ...
 *
 * 呼叫時機：在 IR_ReadSide() 之後，因為依賴 IR[0] 和 IR[12] 是最新值
 * 取樣頻率建議：100~500 Hz（marker 通常 5 mm 寬，3 m/s 通過約 1.7 ms，
 *               所以 500 Hz 完全夠用）
 */
void IR_DetectMarker(void)                                   // 對外 API：依 IR[0]/IR[12] 偵測 marker 通過事件
{                                                            // 函式起始
    /* === 左 marker (IR[0] = IR_L) === */
